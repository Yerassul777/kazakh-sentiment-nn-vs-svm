...
**Class balancing:** random oversampling меньшего класса до паритета, только на train; validation и test сохраняют исходное распределение.

Все 140 126 исходных строк остаются в выборке всегда, дубликаты негативного класса только добавляются сверху — до 110 417 в каждом классе, 220 834 строки всего, из них 80 708 дубликатов. Наивная реализация (взять `target` строк с возвращением одним вызовом) может случайно потерять часть исходных строк — здесь так не сделано.

---

## Признаки

Текст превращается в числа через TF-IDF: вес терма тем больше, чем чаще он в этом отзыве и чем реже во всей коллекции.

$$\large \text{tfidf}(i, t) = \text{tf}(i, t)\cdot\log\frac{N}{\text{df}(t)}$$

где tf — сколько раз терм встретился в отзыве, df — в скольких отзывах он есть всего, N — число отзывов. Настройка sublinear_tf заменяет tf на 1 + log(tf), чтобы десятикратный повтор слова не давал десятикратный вес.

![Как строятся признаки](FEATUR~1.PNG)

| Блок | Параметры | Размер |
|---|---|---|
| Слова | ngram_range=(1,2), min_df=3 | 15 000 |
| Символы | analyzer="char_wb", ngram_range=(3,5) | 15 000 |
| Итого | без сжатия размерности | 30 000 |

Порог min_df=3 оставляет в словаре только термы, встретившиеся минимум в трёх отзывах, — так отсекаются опечатки-одиночки.

**Зачем нужны символьные блоки.** Казахский агглютинативный: кітап, кітабы, кітапты — три разных слова для словаря, но подстроку кіта они делят, и модель видит их родство. Второе: опечатка портит два-три куска из десятка, тогда как на уровне слова она создаёт новый неизвестный токен. Транслитерацию это не лечит — у keremet и керемет нет ни одного общего символа.

Пары слов ловят отрицание: жаман — плохо, жаман емес — неплохо, и это самый сильный положительный вес во всей модели, +1.238.

**Data leakage.** Словарь и IDF-веса строятся только по train. Validation и test проходят через уже готовый словарь. Если построить его сразу по всем данным, статистика тестовых текстов попадёт в обучение, и итоговая оценка окажется завышенной — модель косвенно увидит то, на чём её проверяют.
...
Матрицы показывают, чем именно куплена разница. Сеть переносит ошибки из FN в FP: пропускает на 253 положительных отзыва меньше (2 048 против 2 301), но ошибочно называет положительными на 74 негативных больше (955 против 881). Суммарно 3 003 ошибки против 3 182 — сеть смещает порог в сторону положительного класса и на этом выигрывает.
...
- rating ставил сам автор без инструкции разметчику, единого стандарта у меток нет
